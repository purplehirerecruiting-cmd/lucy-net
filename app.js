...
function moscowParts(date){
 const parts=new Intl.DateTimeFormat("ru-RU",{timeZone:MOSCOW_TZ,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:false}).formatToParts(date);
...
function formatPublished(iso){
 const published=new Date(iso),now=new Date(),p=moscowParts(published),n=moscowParts(now),diff=dayNumber(n)-dayNumber(p),time=`${p.hour}:${p.minute}`;
...
if(pinned){
 ...
 label.innerHTML="<span>закреплено</span><span>10 стикеров</span>";
 pinned.prepend(label);
 const text=pinned.querySelector(".post-text");
 if(text){
  const heading=document.createElement("div");
  heading.className="sticker-heading";
  heading.textContent="Стикеры ЛЮСИ.НЕТ";
  text.before(heading);
  const preview=document.createElement("div");
  preview.className="sticker-preview";
  preview.setAttribute("aria-label","Превью стикеров Люсьена");
  ["КОФЕ","РАБОТА","ЛЮДИ","РЕЖИМ ВЫЖИВАНИЯ"].forEach(labelText=>{
...
if(worlds){
 ...
 moon.className="world-symbol";
 moon.setAttribute("aria-hidden","true");
 moon.textContent="☾";
 worlds.prepend(moon);
 const bonus=document.createElement("section");
 bonus.id="bonuses";
 bonus.className="utility-card bonus-card";
...
if(!document.querySelector('link[rel="canonical"]')){
 const canonical=document.createElement("link");canonical.rel="canonical";canonical.href="https://lusy.space/";document.head.appendChild(canonical);
}
